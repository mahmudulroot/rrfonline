...
function uxstock_slides_shortcode($atts){
      ...
      $list = '
        <script>
            jQuery(window).load(function(){
                jQuery(".uxstock_slides").owlCarousel({
                    items:1,
                    loop:'.$loop.',
                    autoplay:'.$autoplay.',
 ...
 if(array_key_exists('overly_color',$slide_meta)){
        ...
        $list .= '
        <div style="background-image:url('.get_the_post_thumbnail_url($idd,'lage').');height:'.$height.'px" class="uxstock-slide-item">;
        ...
        $list.='<a href="'.$btn_link.'" class="'.$button['type'].'-btn uxstock-slide-btn">'.$button['text'].'</a>';
         }
